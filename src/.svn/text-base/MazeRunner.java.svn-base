//UIUC CS125 FALL 2012 MP. File: MazeRunner.java, CS125 Project: Challenge7-RecursiveKnight, Version: 2012-11-09T13:47:34-0600.198435000
/**
 * 
 * @author dpham9
 *
...
public class MazeRunner {
	...
	static int findShortestString(String[] paths, int lo, int hi) {
		if(lo == hi) return lo;
		return (safeStringLength(paths[lo]) < safeStringLength(paths[findShortestString(paths, lo +1, hi)]) ? lo:findShortestString(paths, lo +1, hi) );
	}
	...
	public static String shortestPath(int x, int y, int tX, int tY,
		...
		if(x >= blocked.length || x < 0 || y < 0 || y >= blocked[0].length) return null;
		...
		blocked[x][y] = true;
		String [] paths = {
				shortestPath(x, y + 1, tX, tY, blocked), shortestPath(x, y -1, tX, tY, blocked), shortestPath(x+1, y , tX, tY, blocked), shortestPath(x-1, y , tX, tY, blocked)
		};
		...
		if(safeStringLength(paths[a]) < Integer.MAX_VALUE){
			if(a == 0 ) result = "N" + paths[a];
			else if(a == 1) result = "S" + paths[a];
			else if(a == 2) result = "E" + paths[a];
			else result = "W" + paths[a];
		}
	...
	public void chase(boolean maze[][], int targetX, int targetY) {
		...
		String result = shortestPath(this.x, this.y, targetX, targetY,maze);
		if(result != null && !result.equals("")) moveOne(result.charAt(0));
		else return;
		//chase(maze, targetX, targetY);
